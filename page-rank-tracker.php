...
// ============================================================
// 初回表示 seed — DB-only 順位データを描画時に直接取得して inline。
// 初回の REST 往復（fetchRankings → /rank-tracker/rankings）を消す。
//
// 注意:
//  - エンドポイントは get_current_user_id() でユーザー解決する。seed を
//    ライブ fetch とバイト同一に保つため、ここでも同一ハンドラを呼び、
//    view-as（mimamori_get_view_user_id）は使わない。
//  - 失敗時・空データ時は何も出さない → JS が従来通り fetch する（冷パス不変）。
//  - 既定スロット 'all' のみ seed。他スロットは通常 fetch。
//  - ?nocache=1 のときは鮮度優先で seed しない。
//  - ワンショット: JS 側の通常キャッシュキー 'rank_tracker_all' に注入するため、
//    add/edit/delete 後の clear('rank_tracker_') や fetchAll 後の再取得では
//    seed は参照されず必ずライブ REST に当たる。
$gcrev_rt_seed = null;
if ( empty( $_GET['nocache'] ) && class_exists( 'Gcrev_Insight_API' ) ) {
    try {
        $gcrev_rt_api  = new Gcrev_Insight_API( false );
        $gcrev_rt_req  = new WP_REST_Request( 'GET', '/gcrev/v1/rank-tracker/rankings' );
        $gcrev_rt_resp = $gcrev_rt_api->rest_get_rank_tracker_rankings( $gcrev_rt_req );
        $gcrev_rt_data = $gcrev_rt_resp->get_data();
        if (
            is_array( $gcrev_rt_data )
            && ! empty( $gcrev_rt_data['success'] )
            && isset( $gcrev_rt_data['data']['keywords'] )
            && is_array( $gcrev_rt_data['data']['keywords'] )
            && count( $gcrev_rt_data['data']['keywords'] ) > 0
        ) {
            $gcrev_rt_seed = $gcrev_rt_data['data'];
        }
    } catch ( \Throwable $e ) {
        $gcrev_rt_seed = null;
    }
}
if ( $gcrev_rt_seed !== null ) :
?>
<script>
(function () {
    try {
        window.__GCREV_SEED = window.__GCREV_SEED || {};
        window.__GCREV_SEED['rank_tracker_all'] = <?php echo wp_json_encode( $gcrev_rt_seed, JSON_UNESCAPED_UNICODE ); ?>;
    } catch (e) {}
})();
</script>
<?php
...
(function() {
    ...
    function fetchRankings() {
        ...
        // 初回のみ: PHP が inline した seed（slot 'all' のみ）を直接参照して
        // 初回 REST 往復を消す。footer の gcrevCache 定義順に依存しない。
        // gcrevCache に当たらなかった場合のみ参照するため、fetchAll 後の
        // clear('rank_tracker_') 以降は seed には当たらず必ずライブ REST に行く。
        ...
        // ワンショット: gcrevCache 命中・seed 直接参照のどちらで消費しても __GCREV_SEED は初回で必ず破棄する。
        // （footer が seed を gcrevCache へ流し込むため初回は gcrevCache 命中側を通る＝上の if は実行されない。
        //   ここで消さないと mutation 後の clear('rank_tracker_') → 再取得が生き残った seed を再表示し stale になる）
        if (window.__GCREV_SEED && window.__GCREV_SEED[cacheKey] !== undefined) {
            try { delete window.__GCREV_SEED[cacheKey]; } catch (e) { window.__GCREV_SEED[cacheKey] = null; }
        }
        if (cached) {
