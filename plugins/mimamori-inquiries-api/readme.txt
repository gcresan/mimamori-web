...
= 1.1.3 =
* 修正: 別コピー等でクラスが先に読み込まれていると、ファイル冒頭の class_exists ガードで早期 return され「設定 → みまもり問い合わせAPI」メニューが登録されない不具合を修正
* 管理メニュー / admin_post ハンドラの登録を REST ルートと同様にファイル先頭（ガードより前）へ移動し、二重ロード環境でも必ず設定画面が出るようにした
