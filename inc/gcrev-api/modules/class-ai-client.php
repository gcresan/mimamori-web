...
class Gcrev_AI_Client {
    ...
    /**
     * Claude (Anthropic) を Vertex AI 経由で呼び出してテキスト応答を返す。
     *
     * Gemini と同じ GCP サービスアカウント／アクセストークンを使う。
     * Vertex 版 Messages API は通常の Anthropic Messages API とほぼ同一だが、
     *   - model は URL に入れる（ボディには入れない）
     *   - ボディに anthropic_version: "vertex-2023-10-16" が必須
     *   - 応答テキストは content[0].text から取得（Gemini の candidates ではない）
     * という3点が異なる。
     *
     * @param  string $prompt  ユーザープロンプト
     * @param  array  $options { system?: string, max_tokens?: int, temperature?: float }
     * @return string Claude の応答テキスト
     * @throws \Exception 通信エラー・空応答の場合
     */
    public function call_claude_vertex( string $prompt, array $options = [] ): string {
        ...
        $location = $this->config->get_claude_vertex_location();
        $model    = $this->config->get_claude_vertex_model();
        ...
        // temperature は Sonnet 系では有効。Opus 4.7/4.8 系に切り替える場合は
        // 送ると 400 になるため、その際はこの分岐ごと外すこと。
        if ( isset( $options['temperature'] ) && strpos( $model, 'opus' ) === false ) {
            $body['temperature'] = (float) $options['temperature'];
        }

        $request_body = wp_json_encode( $body, JSON_UNESCAPED_UNICODE );
        $max_retries  = 2;          // 429/503 時の最大リトライ回数
        $retry_waits  = [ 8, 20 ];  // リトライ前の待機秒数
        $status       = 0;
        $raw          = '';
        $json         = null;
        ...
        for ( $attempt = 0; $attempt <= $max_retries; $attempt++ ) {
            ...
            if ( is_wp_error( $response ) ) {
                throw new \Exception( 'Claude (Vertex) 通信エラー: ' . $response->get_error_message() );
            }
            ...
            // 429 (overloaded/rate limit) / 503 は一時的なので待機してリトライ
            if ( ( $status === 429 || $status === 503 || $status === 529 ) && $attempt < $max_retries ) {
                $wait = $retry_waits[ $attempt ] ?? 20;
                file_put_contents( '/tmp/gcrev_review_debug.log',
                    date( 'Y-m-d H:i:s' ) . " call_claude_vertex: HTTP {$status} (rate limited), retry " . ( $attempt + 1 ) . "/{$max_retries} after {$wait}s\n",
                    FILE_APPEND
                );
                sleep( $wait );
                continue;
        ...
        if ( $status < 200 || $status >= 300 ) {
            $msg = $json['error']['message'] ?? ( $json['message'] ?? $raw );
            file_put_contents( '/tmp/gcrev_review_debug.log',
                date( 'Y-m-d H:i:s' ) . " call_claude_vertex: API ERROR HTTP {$status}: " . substr( $raw, 0, 500 ) . "\n",
                FILE_APPEND
            );
            if ( $status === 429 || $status === 529 ) {
                throw new \Exception( "Claude API エラー (HTTP {$status}): アクセスが集中しています。1〜2分ほど待ってから、もう一度お試しください。（" . $msg . "）" );
            }
            throw new \Exception( "Claude (Vertex) API エラー (HTTP {$status}): " . $msg );
        }

        // 応答テキストを抽出（content は text ブロックの配列）
        ...
        if ( $text === '' ) {
            file_put_contents( '/tmp/gcrev_review_debug.log',
                date( 'Y-m-d H:i:s' ) . " call_claude_vertex: empty text, stop_reason={$stop_reason}, body=" . substr( $raw, 0, 500 ) . "\n",
                FILE_APPEND
            );
            throw new \Exception( 'Claude の応答が空でした。stop_reason=' . $stop_reason );
        }
