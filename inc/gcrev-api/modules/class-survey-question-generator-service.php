...
class Gcrev_Survey_Question_Generator_Service {
     ...
     * @param int   $target_count 生成する質問数（1〜30）。既存の残り枠に合わせて指定する
     * @return array { success, questions, design_intent, industry_label, message, raw }
     */
    public function generate( array $input, int $target_count = 30 ): array {
        ...
        // 生成数のバリデーション（1〜30の範囲、デフォルト30）
        $target_count = max( 1, min( 30, $target_count ) );
        ...
        try {
            $prompt = $this->build_prompt( $industry, $service_description, $target, $strengths, $review_emphasis, $target_count );
            ...
            $questions = $this->normalize_questions( $parsed['questions'], $parsed['fixed_ids'] ?? [], $target_count );
    ...
    /**
     * 指定件数に応じてカテゴリ配分をスケールダウンする。
     * 30問 = オリジナル、それ未満は比率で縮小（自由回答は必ず2問確保）。
     */
    private function build_scaled_category_plan( int $target_count ): array {
        if ( $target_count >= 30 ) {
            return self::CATEGORY_PLAN;
        }

        // 自由回答は 2 問確保（required + optional）
        $freeform_count = ( $target_count >= 2 ) ? 2 : $target_count;
        $remaining_target = $target_count - $freeform_count;

        // 自由回答以外のカテゴリ
        $other_categories = self::CATEGORY_PLAN;
        unset( $other_categories['自由回答'] );
        $other_total = array_sum( $other_categories ); // 28

        $scaled = [];
        $accumulated = 0;
        foreach ( $other_categories as $cat => $count ) {
            if ( $remaining_target <= 0 ) { break; }
            // 比率でスケールするが、最低 1 保証（余裕がある場合のみ）
            $scaled_count = (int) max( 0, round( $count * $remaining_target / $other_total ) );
            if ( $scaled_count > 0 ) {
                $scaled[ $cat ] = $scaled_count;
                $accumulated += $scaled_count;
            }
        }

        // 端数調整: 合計が $remaining_target に達するまで残りカテゴリに +1
        $diff = $remaining_target - $accumulated;
        if ( $diff !== 0 ) {
            foreach ( $other_categories as $cat => $_ ) {
                if ( $diff === 0 ) { break; }
                if ( $diff > 0 ) {
                    $scaled[ $cat ] = ( $scaled[ $cat ] ?? 0 ) + 1;
                    $diff--;
                } elseif ( isset( $scaled[ $cat ] ) && $scaled[ $cat ] > 1 ) {
                    $scaled[ $cat ]--;
                    $diff++;
                }
            }
    ...
    private function build_prompt(
        ...
        $plan = $this->build_scaled_category_plan( $target_count );
        $actual_total = array_sum( $plan );

        $category_spec = [];
        foreach ( $plan as $cat => $count ) {
...
以下の情報をもとに「自然でバリエーションのある口コミを生成するためのアンケート{$actual_total}問」を設計してください。
...
# 事実に基づく設計（最重要・厳守）
このアンケートは「実際に利用した人」が答えるものです。提供企業が言っていない・提供していないことを、あたかも事実であるかのように質問の前提にしてはいけません。回答者が「そんなこと言われていない／されていない」と感じる質問は、無理に褒めさせられている印象を与えるため厳禁です。
- 上記の「入力情報」に書かれていない具体的なサービス・設備・対応内容・実績・受賞・数値・専門性を、質問の中に新たに作り出さない（オーバースペック化の禁止）
- 「強み」欄に挙がっていない長所を、質問の前提にしない（例: 強みに「夜間対応」がなければ夜間対応を前提とする質問を作らない）
- 入力情報が乏しい場合は、具体を創作して埋めず、一般的・中立的な聞き方にとどめる
- 質問は出来事や事実を中立に尋ねる形にする。「良かった点はどこですか」のように好評価を前提とせず、「対応はいかがでしたか」のように良し悪しを回答者が自由に判断できる聞き方にする
- 「業界No.1」「最高」「完璧」「感動」などの誇張語・最上級表現を質問文や選択肢に使わない
- 褒め言葉を回答者の口に入れる（言わせる）ような誘導はしない。あくまで体験を素直に書ける質問にする
...
radio/checkbox の options は 4〜7 個の具体的・業種特化した選択肢を入れる（回答者が「あるある」と共感できる言葉選び）。ただし入力情報にない事柄を選択肢として作り出さないこと。
- 選択肢は肯定一辺倒にせず、回答者が正直に答えられる逃げ道を必ず用意する。
  - radio の評価・満足度スケールは「とても良い〜あまり良くない／不満」のように中立〜低評価まで含む段階にする（高評価だけを並べない）
  - checkbox には末尾に「特になし」「特に感じなかった」「該当しない」のいずれかを必ず1つ加える
  - 必要に応じて「その他」も加えてよい
...
- 回答例は「褒め」を強制しない中立的な書き方にする。良い体験を断定する例文ではなく、事実や感じたことを素直に書けるトーンにする（入力情報にない長所を例文に盛り込まない）
- 例（歯科医院の場合）: 「例：治療前はどの歯科にするか迷っていました。実際に通ってみて感じたことや、印象に残ったことを自由にお書きください。」
...
# 固定質問の選定
- {$actual_total}問のうち 2〜4 問を「口コミ生成に必須の固定質問」として選ぶ（決め手・総合評価・自由回答・おすすめのいずれかを含める、件数が少ない場合は2問でよい）
...
# 厳守事項
- questions は必ず {$actual_total} 件、カテゴリ順で並べる
- 質問文に個人名・企業名・具体URLは含めない
- 「満足しましたか？」のような定型質問は禁止
- どの業種にも当てはまる汎用質問は禁止
- 入力情報にない具体的な長所・サービス・実績を質問や選択肢に作り出さない（オーバースペック化の禁止）
- 好評価を前提とした誘導表現・誇張語を使わず、回答者が正直に書ける中立的な質問にする
- 自由回答カテゴリの最後の1問は「最後に何か一言お願いします」系の自由欄にする
PROMPT;
    }
     ...
     */
    private function normalize_questions( array $raw_questions, array $fixed_ids, int $target_count = 30 ): array {
        ...
        // target_count を超えた分は切り詰める（AI が多めに返した場合の保険）
        if ( $target_count > 0 && count( $out ) > $target_count ) {
            $out = array_slice( $out, 0, $target_count );
        }
