...
PROMPT;
    ...
    public function get_or_generate_action_guide( int $user_id, int $action_id ): array {
        ...
        // 既存キャッシュが「途中で切れている疑い」「旧フォーマット」のときは再生成する。
        $looks_outdated_or_truncated = function( string $t ): bool {
            if ( $t === '' ) return false;
            // 旧フォーマット（「やる手順」「所要時間」「なぜこれが必要か」を含むもの）→ 再生成
            if ( mb_strpos( $t, 'やる手順' ) !== false )       return true;
            if ( mb_strpos( $t, '所要時間' ) !== false )       return true;
            if ( mb_strpos( $t, 'なぜこれが必要か' ) !== false ) return true;
            // 必須セクション「何を直すか」が欠けていれば不完全
            if ( mb_strpos( $t, '何を直すか' ) === false )     return true;
            // 末尾が句読点/閉じカッコ/改行で終わっていない → 途中切れ
            if ( ! preg_match( '/[。！？\)\）\]\】」』\.\!\?\n]\s*$/u', $t ) ) return true;
            return false;
        };

        if ( $existing !== '' && ! $looks_outdated_or_truncated( $existing ) ) {
            return [
                'success'    => true,
    ...
    private function generate_action_guide_with_ai( int $user_id, array $action ): string {
        ...
        $prompt = <<<PROMPT
あなたは中小企業オーナーに「今月やるべきWeb改善施策」を提示する
コンテンツ／SEO の専門家です。

下記の改善施策について、「ページの何を・どこに・どう直すか」が
具体的に分かるアドバイス記事を Markdown で書いてください。
...
【絶対に書かないこと（重要）】
- WordPress / 管理画面の使い方（ログイン手順、投稿の新規追加方法、
  プラグインのインストール、メニュー操作など）の解説は一切不要。
- 「管理画面にログインしてください」「投稿一覧を開いてください」など、
  ツール操作のステップは書かない。実装は別の人がやる前提でよい。
- 「WordPress」「管理画面」「ダッシュボード」「プラグイン」など
  ツール名・画面名は本文に登場させない。

【書くべきこと】
- ページのどの要素を、どう変えるか の **コンテンツ／SEO 上の具体策**
- 例: 「ファーストビューに〇〇という見出しを入れる」「H2 に
  『△△の流れ』『△△の費用』『よくある質問』の 3 セクションを追加」
  「タイトルタグを『産婦人科 コンサルティング | 〇〇社』に変更」
  「『お問い合わせ』ボタンを最初の画面の右上に配置」など
- ターゲットキーワードや競合状況を踏まえた具体的な改善内容

【書き方ルール】
- 専門用語は最小限。使う場合は短くカッコ書きで補足
  （例: 「H1（ページの一番上の大きな見出し）」）
- 「〜しましょう」「〜してください」など丁寧な語り口
- **全体 300〜500 文字程度。短く端的に。前置き・繰り返し・励ましの言葉は禁止**
- 1項目1アクション。1項目に複数の指示を詰め込まない
- Markdown 構造は以下のテンプレートに必ず従う：

## 何を直すか
- 箇条書き 3〜5 個・各 60〜100文字
- ページのどこを、どう変えるか を1項目ずつ具体名で
- 良い例: 「タイトルタグを『産婦人科コンサルティング｜会社名』に変更」
- 良い例: 「H2 に『費用の目安』『相談の流れ』『よくある質問』の 3 セクションを追加」
- 良い例: 「ページ最上部に『無料相談はこちら』ボタンを配置」
- 悪い例（禁止）: 「コンテンツを充実させる」「SEO を強化する」「UX を改善する」

## 含めるべき内容
- 箇条書き 2〜3 個・各 50文字以内
- 検索ユーザーが知りたい具体的トピックや関連キーワード
- このセクションが不要な施策なら省略可

【絶対ルール】
- 出力は Markdown のみ。前後の説明文・コードブロック記法は禁止
- 「なぜ必要か」「仕上がりの目安」「所要時間」など補足セクションは書かない
- 架空の URL・競合名・数値を作らない
- 順位や数値の捏造禁止。施策の reason に書かれている範囲で語る
PROMPT;

        $response = $this->ai->call_gemini_api( $prompt, [
            'temperature'     => 0.4,
            'maxOutputTokens' => 8192, // 日本語1文字≈2トークン、800〜1200文字+構造マークアップで2400トークン程度。安全マージンを十分に取る
        ] );
