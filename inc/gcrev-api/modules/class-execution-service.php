...
PROMPT;
    ...
    public function get_or_generate_action_guide( int $user_id, int $action_id ): array {
        ...
        // 既存キャッシュが「途中で切れている疑い」がある場合は再生成する。
        // 判定: 必須セクション「所要時間」を含まない、または末尾が句読点/改行で終わらず途中で切れている。
        $looks_truncated = function( string $t ): bool {
            if ( $t === '' ) return false;
            if ( mb_strpos( $t, '所要時間' ) === false ) return true; // 最終セクション欠落
            if ( ! preg_match( '/[。！？\)\）\]\】」』\.\!\?\n]\s*$/u', $t ) ) return true; // 末尾が中途半端
            return false;
        };

        if ( $existing !== '' && ! $looks_truncated( $existing ) ) {
            return [
                'success'    => true,
...
- 全体は 800〜1200 文字程度（必ずこの範囲に収め、超過しないこと）
- 各セクションは下記の文字数目安を厳守し、冗長な前置きや繰り返しは禁止
- Markdown 構造は以下のテンプレートに必ず従う：

## なぜこれが必要か
（2〜3文・150文字以内。データや状況を踏まえて簡潔に。前置きは不要）

## やる手順
1. 何を、どこで、どう操作するか（1ステップ 80文字以内）
2. ボタン名・画面名は実際の名称で（例: 「WordPress管理画面 → 投稿 → 新規追加」）
3. つまずきそうな点があれば1文だけ補足
4. ...（合計 4〜6 ステップ）
...
- 全セクションを必ず最後まで書き切ること（途中で切れた応答にならないよう短く端的に）
PROMPT;
