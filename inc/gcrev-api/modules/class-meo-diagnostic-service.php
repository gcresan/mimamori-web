...
```json
{
  "summary": "総合評価の本文。下記の構成要素を順に、段落ごとに改行(\\n)で区切りながら2,300文字程度の十分な分量で詳しく記述する。",
...
## summary（総評）の書き方（重要）
summary は最も重要な項目です。必ず **2,300文字程度** の読み応えのある文章にし、以下の構成要素をこの順序で、それぞれ段落（改行 \\n で区切る）として記述してください。各段落は単なる箇条書きではなく、理由や背景を添えた丁寧な文章にすること。

1. **総括**: 総合スコアとグレードを示し、Googleビジネスプロフィール全体の現状を経営者にも分かる言葉で総評する。
2. **カテゴリ別の評価**: 基本情報・投稿・写真・レビューの4カテゴリについて、それぞれのスコアを踏まえて「何が良く、何が足りないのか」「なぜそのスコアなのか」を具体的に説明する。
3. **口コミの評価**: 口コミ数・平均評価・返信率を踏まえ、集客や信頼性の観点から現状を評価し、返信や口コミ獲得の重要性に触れる。
4. **検索順位の評価**: 主要キーワードのマップ順位を踏まえ、現在の集客力と上位表示に向けた課題を説明する。
5. **強み**: このプロフィールが既に持っている強み・活かすべき点をまとめる。
6. **改善すべき点**: 優先的に直すべき弱点と、それを放置した場合の影響を説明する。
7. **今後の方針**: 次の一手として何から着手すべきか、向こう1〜3ヶ月の取り組み方針を前向きにまとめる。
...
- summary は途中で切れないよう、2,300文字程度を最後まで書き切る
PROMPT;
        ...
        try {
            $response = $this->ai->call_gemini_api( $prompt, [
                'temperature'      => 0.3,
                // 総評を2,300文字程度の長文にするため出力上限を拡張
                // （日本語2,300文字＋good/improvement/recommendations を収めるには十分な余裕が必要）
