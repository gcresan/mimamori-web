...
以下のアクセスデータをもとに、{$year}年の年間総括をサイト管理者・運営担当者向けに書いてください。
...
以下4点を■見出し付きで簡潔にまとめてください（合計300〜500文字）。

■この1年の成果
数値の変化を端的に報告する。具体的な数字を引用し、増減を明確に示す。

■特に伸びたポイント
何が伸びたか、なぜそう言えるかを短く整理する。事実ベースで簡潔に。

■課題
何が弱いのかを明確にする。「新規ユーザー数がXX%減少」のように具体的に。

■来年の改善ポイント
「〜を強化するとよい」「〜の見直しが必要」のように、管理者が次に動ける改善提案を書く。

【文章ルール】
- 読み手はサイト管理者・運営担当者。一般ユーザー向けではない
- 感謝・語りかけ・情緒的な表現は不要（「ご利用ありがとう」「前向きに取り組みます」等はNG）
- 「〜しました」「〜が見られます」「〜が必要です」のような客観的な報告文で書く
- 専門用語は避け、やさしい日本語で簡潔に
