...
add_action('after_setup_theme', function () {
    ...
    gcrev_analysis_units_create_table();
...
// =========================================
// 解析ユニット（Multi Analysis Unit）
// 1クライアントに複数の集客導線（LP/MEO等）を紐づける
// =========================================

/**
 * 解析ユニットテーブル作成
 */
function gcrev_analysis_units_create_table(): void {
    global $wpdb;

    $table = $wpdb->prefix . 'gcrev_analysis_units';
    $charset_collate = $wpdb->get_charset_collate();
    ...
    $sql = "CREATE TABLE {$table} (
        ...
        unit_type VARCHAR(10) NOT NULL DEFAULT 'web',
        ga4_property_id VARCHAR(20) DEFAULT NULL,
        page_path_prefix VARCHAR(255) DEFAULT NULL,
        gsc_url VARCHAR(500) DEFAULT NULL,
        meo_domain VARCHAR(255) DEFAULT NULL,
        meo_location_id VARCHAR(100) DEFAULT NULL,
        sort_order INT NOT NULL DEFAULT 0,
        enabled TINYINT(1) NOT NULL DEFAULT 1,
        created_at DATETIME NOT NULL,
        updated_at DATETIME NOT NULL,
 ...
 * @return array ユニット一覧（0件なら空配列＝従来の単一サイトユーザー）
 */
function gcrev_get_analysis_units( int $user_id ): array {
    global $wpdb;
    ...
    $results = $wpdb->get_results(
        $wpdb->prepare(
            "SELECT * FROM {$table} WHERE user_id = %d AND enabled = 1 ORDER BY sort_order ASC, id ASC",
            $user_id
        ),
        ARRAY_A
    );
...
/**
 * ユーザーが解析ユニットを持っているか判定
 *
 * @param int $user_id ユーザーID
 * @return bool 1件以上あれば true
 */
function gcrev_has_analysis_units( int $user_id ): bool {
    global $wpdb;
    ...
    $count = (int) $wpdb->get_var(
        $wpdb->prepare(
            "SELECT COUNT(*) FROM {$table} WHERE user_id = %d AND enabled = 1",
            $user_id
        )
    );
...
/**
 * ユニット設定を解決（unit_id=0 なら既存設定にフォールバック）
 *
 * @param int $user_id  ユーザーID
 * @param int $unit_id  ユニットID（0 = 全体/フォールバック）
 * @return array 設定配列
 */
function gcrev_resolve_unit_config( int $user_id, int $unit_id = 0 ): array {
    // デフォルト（既存の単一サイト設定）
    $default = [
        'unit_id'          => 0,
        'unit_type'        => 'web',
        'label'            => '全体',
        'slug'             => 'all',
        'ga4_property_id'  => trim( (string) get_user_meta( $user_id, 'ga4_property_id', true ) ),
        'gsc_url'          => trim( (string) get_user_meta( $user_id, 'weisite_url', true ) ),
        'page_path_prefix' => null,
        'meo_domain'       => get_user_meta( $user_id, '_gcrev_maps_domain', true ) ?: '',
        'meo_location_id'  => null,
    ];

    // unit_id=0 or ユニット未指定 → デフォルト
    if ( $unit_id <= 0 ) {
        return $default;
    }
    ...
    $unit = $wpdb->get_row(
        $wpdb->prepare(
            "SELECT * FROM {$table} WHERE id = %d AND user_id = %d AND enabled = 1",
            $unit_id,
            $user_id
        ),
        ARRAY_A
    ...
    if ( ! $unit ) {
        return $default; // 見つからなければフォールバック
    }

    return [
        'unit_id'          => (int) $unit['id'],
        'unit_type'        => $unit['unit_type'] ?: 'web',
        'label'            => $unit['label'] ?: '',
        'slug'             => $unit['slug'] ?: '',
        'ga4_property_id'  => $unit['ga4_property_id'] ?: $default['ga4_property_id'],  // NULL=継承
        'gsc_url'          => $unit['gsc_url'] ?: $default['gsc_url'],                  // NULL=継承
        'page_path_prefix' => $unit['page_path_prefix'] ?: null,
        'meo_domain'       => $unit['meo_domain'] ?: $default['meo_domain'],
        'meo_location_id'  => $unit['meo_location_id'] ?: null,
    ];
}
