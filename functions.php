...
function gcrev_ensure_meo_report_page(): void {
    ...
    $existing = get_page_by_path( 'meo-report' );
    if ( $existing ) {
        if ( get_post_meta( $existing->ID, '_wp_page_template', true ) !== 'page-meo-report.php' ) {
            update_post_meta( $existing->ID, '_wp_page_template', 'page-meo-report.php' );
        }
        update_option( 'gcrev_meo_report_page_id', (int) $existing->ID, false );
        return;
    }
    $page_id = wp_insert_post( [
        'post_type'    => 'page',
        'post_status'  => 'publish',
        'post_title'   => 'MEOレポート',
        'post_name'    => 'meo-report',
        'post_content' => '',
        'meta_input'   => [ '_wp_page_template' => 'page-meo-report.php' ],
    ] );
    if ( ! is_wp_error( $page_id ) && $page_id > 0 ) {
...
if ( defined( 'WP_CLI' ) && WP_CLI ) {
    ...
    if ( file_exists( $meo_cli_path ) ) {
        ...
        WP_CLI::add_command( 'mimamori meo probe', [ $meo_cli, 'probe' ] );
    }
}
...
if ( ! function_exists( 'mimamori_generate_monthly_report_with_claude' ) ) {
    function mimamori_generate_monthly_report_with_claude( array $report_data, array $options = [] ) {
        ...
        // ------------------------------------------------------------
        // easy mode 判定 (報告者向けの語調コントロール)
        // ------------------------------------------------------------
        $is_easy_mode = ( ( $report_data['output_mode'] ?? 'normal' ) === 'easy' )
            || ! empty( $options['easy_mode'] );

        $tone_block = $is_easy_mode
            ? <<<EASY
【今回の語調 — 初心者向けモード】
- 読み手は Web に詳しくない経営者・現場担当者です
- 専門用語は使わず、使う場合は必ずカッコで短く補足する（例: CV(問い合わせなどの成果)、CTR(クリック率)、SEO(検索からの集客)）
- 比喩や日常的な言い回しで「数字の意味」を実感できるように説明する
- 不安を煽らず「〜の可能性があります」「〜の傾向が見えます」と前向きに
- 各文は短く、1 段落 2〜3 文程度
EASY
            : <<<NORMAL
【今回の語調 — 通常モード】
- 読み手は中小企業の経営者・Web 担当者です
- 必要に応じて専門用語を使ってよい（ただし初出では補足する）
- 数値の解釈・優先度判断・施策の根拠まで踏み込む
NORMAL;
...
【月次レポート設定で入力された内容 (client フィールド) への直接応答 — 厳守】
入力データの `client` 直下に、ユーザーが月次レポート設定画面で書いた情報が
入っています。次のフィールドが「空でない」場合は、必ずレポート内で直接応答してください。

- `client.issue` (先月の課題): その課題が今月のデータで解消／継続／悪化のどれかを必ず判定し、考察や警告に反映する
- `client.monthly_goal` (先月の目標): 達成度を数値で評価する (達成 / 未達 / 進捗◯%)。next_actions の優先順位もここを軸にする
- `client.focus_numbers` (重視している数字): この指標を最重要 KPI として overall_summary や key_findings の中心に据える
- `client.current_state` (現在のサイトの状況): クライアントの認識を前提に置き、データで補強または訂正する
- `client.site_goal` (大きな目標): 長期目標との整合性 (今月の動きはこの目標に近づいているか) を必ず触れる
- `client.additional_notes` (追加で伝えたい情報): 文脈情報なので、関連する考察箇所で必ず反映する

これらに記入がある場合、レポートが「ユーザーが書いた質問に直接答える形」になるようにしてください。
記入がない (空文字) フィールドについては触れる必要はありません。

【過去の深掘りレポート (strategy_history) の活用】
入力データに `strategy_history` 配列が存在する場合:
- これは過去に同じクライアントに対して生成された「深掘りレポート (戦略提案)」の要約です
- 最新のものから順に並んでいます
- 各要素は year_month / alignment_score / conclusion / issues_titles / this_month_todos / recommended_actions を持ちます
- 今月の月次レポートを作る際は、過去の推奨アクション (recommended_actions / this_month_todos) が今月の数値にどう影響したか・進捗があるかを必ず触れてください
- 過去に指摘された課題 (issues_titles) が解消されたか・継続しているか・新たに発生しているかを観察してください
- 過去レポートとの整合性が取れた「継続支援レポート」になるように、新規発見だけでなく前回提案の振り返り視点を入れてください
- ただし過去レポートに書いてあった事実を丸ごとコピペするのは禁止。今月のデータで再確認・補強・修正する形で言及してください
...
TXT;
        ...
        // PRETTY_PRINT は入力トークンを 30% 増やすので付けない）
        // ------------------------------------------------------------
        $payload_json = wp_json_encode( $report_data, JSON_UNESCAPED_UNICODE );
        ...
        // ------------------------------------------------------------
        // 出力切り詰め検知（stop_reason=max_tokens は JSON 不完全＝パース失敗確定）
        // ------------------------------------------------------------
        $stop_reason = (string) ( $result['stop_reason'] ?? '' );
        if ( $stop_reason === 'max_tokens' ) {
            file_put_contents(
                '/tmp/gcrev_claude_debug.log',
                date( 'Y-m-d H:i:s' ) . ' [helper] OUTPUT TRUNCATED (max_tokens) — increase max_tokens'
                . ' (current=' . ( $options['max_tokens'] ?? 'default' ) . ')' . "\n",
                FILE_APPEND
            );
            return new WP_Error(
                'mimamori_claude_truncated',
                'Claude の出力が max_tokens (' . ( (int) ( $api_options['max_tokens'] ?? 0 ) ) . ') で打ち切られました。max_tokens を増やしてください。'
            );
        }
        ...
        if ( ! is_array( $decoded ) ) {
            file_put_contents(
                '/tmp/gcrev_claude_debug.log',
                date( 'Y-m-d H:i:s' ) . ' [helper] JSON parse failed (stop=' . $stop_reason . '): ' . substr( $text, 0, 500 ) . "\n",
                FILE_APPEND
            );
            return new WP_Error( 'mimamori_claude_parse_failed', 'Claude 応答の JSON パースに失敗しました (stop=' . $stop_reason . ')' );
        }
